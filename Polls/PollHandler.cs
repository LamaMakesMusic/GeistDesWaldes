...
public class PollHandler : BaseHandler
{
    ...
    private readonly object _pollListLock = new();
    private readonly object _pollsLock = new();
    ...
    public async Task<CustomRuntimeResult<Poll>> StartPoll(string name, string description, IChannel channel, string[] options)
    {
        try
        {
            ...
            Poll poll = new(name, description, channel, voteOptions.ToArray());
    ...
    {
        var loadedPolls = await GenericXmlSerializer.LoadAsync<List<ChannelPoll>>(Server.LogHandler, POLLS_FILE_NAME, Server.ServerFilesDirectoryPath);
