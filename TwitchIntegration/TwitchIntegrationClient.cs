    ...
    public class TwitchIntegrationClient
    {
        ...
        private async Task CreateEventSubscriptionsAsync()
        {
            ...
            foreach ((string subType, string version) in _subscriptionTypes)
            {
                ...
                {
                    TwitchIntegrationHandler.LogToMain($"[{ChannelName}] {nameof(OnWebsocketConnected)}", $"Sending Topics '{subType} v.{version}' Failed!", severity: LogSeverity.Error, exception: ex);
                }
            }
        ...
        class ConfigStreamEntity
        {
            public readonly Server Server;
            private readonly ServerConfiguration _config;

            private readonly Dictionary<string, long> _userCooldownList = new Dictionary<string, long>();
            private readonly Lock _userCooldownLocker = new();

            private readonly List<ActiveChatterInfo> _activeChatters = new List<ActiveChatterInfo>();
