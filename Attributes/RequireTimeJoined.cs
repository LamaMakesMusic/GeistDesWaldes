...
public class RequireTimeJoined : PreconditionAttribute
{
    public RequireTimeJoined(string minutes, string hours = "0", string days = "0", string months = "0", string years = "0")
    {
        ...
        {
            TimeJoined.Minutes = minute;
        }

        int hour = 0;
        if (int.TryParse(hours, out hour))
        {
            TimeJoined.Hours = hour;
        }
        ...
        {
            TimeJoined.Months = month;
        }

        int year = 0;
        if (int.TryParse(years, out year))
        {
            TimeJoined.Years = year;
        }
    }

    public TimeObject TimeJoined { get; } = new();
    ...
    public override async Task<PreconditionResult> CheckPermissionsAsync(ICommandContext context, CommandInfo command, IServiceProvider services)
    {
        ...
        {
            await logger.Log(new LogMessage(LogSeverity.Debug, nameof(CheckPermissionsAsync), $"Granted {nameof(RequireTimeJoined)}-Permission for '{nameof(ConsoleMessageChannel)}' -> Required: {TimeJoined}"), (int)ConsoleColor.Green);
        ...
        if (userInQuestion is IGuildUser gUser)
        {
            ...
            {
                TimeObject joinTime = new(DateTime.UtcNow - joinedAt.Value.UtcDateTime, joinedAt.Value.UtcDateTime);

                if (TimeJoined.LessEqualOrGreaterThan(joinTime) != 1)
                {
                    await logger.Log(new LogMessage(LogSeverity.Debug, nameof(CheckPermissionsAsync), $"Granted {nameof(RequireTimeJoined)}-Permission for '{command.Name}' -> Required: {TimeJoined} vs. {joinTime}"), (int)ConsoleColor.Green);

                    return PreconditionResult.FromSuccess();
                }

                errorReason = $"User needs to be a joined member of this guild for at least '{TimeJoined}' to use this command.";
            }
            else
            {
