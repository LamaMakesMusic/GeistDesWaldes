...
namespace GeistDesWaldes.Attributes
{
    public class IndexValuePairReader : TypeReader
    {
        ...
        {
            var splitStringResult = await ArrayReader.SplitToArray(input, new char[] { DEFAULT_SEPERATOR });
    ...
    public class IndexValuePairArrayReader : TypeReader
    {
        public override async Task<TypeReaderResult> ReadAsync(ICommandContext context, string input, IServiceProvider services)
        {
            var inputToArrayResult = await ArrayReader.SplitToArray(input);
            if (inputToArrayResult.IsSuccess)
            {
                ...
                else
                {
                    ...
                    {
                        var lineResult = await IndexValuePairReader.StringToIndexValuePair(line);

                        if (!lineResult.IsSuccess)
                            return lineResult;
                        else if (lineResult.BestMatch is IndexValuePair readPair && readPair != null)
                            pairs.Add(readPair);
                    }
