...
public class MixedUserReader<T> : TypeReader where T : class, IUser
{
    ...
    private async Task<TypeReaderResult> FindTwitchUser(string input)
    {
        ...
        GetUsersResponse userResponse = await TwitchIntegrationHandler.ValidatedApiCall(TwitchIntegrationHandler.Instance.Api.Helix.Users.GetUsersAsync(logins: new List<string> { input }));

        StringBuilder errorReason = new($"Could not find matching twitch user. '{userResponse?.Users?.Length}' results: ");
    ...
    {
        //By Mention (1.0)
        if (MentionUtils.TryParseUser(input, out ulong id) && guild.GetUser(id) is T tUserMentioned && tUserMentioned != null)
        {
