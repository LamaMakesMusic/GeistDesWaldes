...
public class RequireIsFollower : PreconditionAttribute
{
    public override async Task<PreconditionResult> CheckPermissionsAsync(ICommandContext context, CommandInfo command, IServiceProvider services)
    {
        ...
        if (userInQuestion is TwitchUser twitchUser)
        {
            ...
            {
                User channelOwner = ConfigurationHandler.RuntimeConfig[twitchChannel.GuildId].ChannelOwner;
    ...
    public static async Task<(Follow, Exception)> IsUserAFollower(string userId, string channelId)
    {
        ...
        try
        {
            GetUsersFollowsResponse result = await TwitchIntegrationHandler.ValidatedApiCall(TwitchIntegrationHandler.Instance.Api.Helix.Users.GetUsersFollowsAsync(fromId: userId, toId: channelId));
