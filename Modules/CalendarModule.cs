...
public class CalendarModule : ModuleBase<CommandContext>, ICommandModule
{
    ...
    [RequireUserPermission(GuildPermission.Administrator, Group = "CalendarAdminPermissions")]
    [RequireUserPermission(GuildPermission.ManageChannels, Group = "CalendarAdminPermissions")]
...
public class BirthdayModule : ModuleBase<CommandContext>, ICommandModule
{
    ...
    public async Task<RuntimeResult> AddBirthday(DateTime date)
    {
        try
        {
            if (Context.Channel is TwitchMessageChannel || Context.Channel is ConsoleMessageChannel)
            {
