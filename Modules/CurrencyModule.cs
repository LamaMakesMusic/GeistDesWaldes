...
public class CurrencyModule : ModuleBase<CommandContext>, ICommandModule
{
    ...
    public async Task<RuntimeResult> GetPointsAsync()
    {
        try
        {
            CustomRuntimeResult<int> getResult = await Server.GetModule<CurrencyHandler>().GetPointsAsync(Context.User);
    ...
    [RequireUserPermission(GuildPermission.Administrator, Group = "CurrencyAdminPermission")]
    [RequireUserPermission(GuildPermission.ManageChannels, Group = "CurrencyAdminPermission")]
    ...
    public class TwitchPointsAdminModule : ModuleBase<CommandContext>, ICommandModule
    {
        ...
        public async Task<RuntimeResult> RemoveTwitchPoints(int amount, IUser targetUser = null)
        {
            try
            {
                ...
                CustomRuntimeResult result = await Server.GetModule<CurrencyHandler>().AddCurrencyToUser(targetUser, -1 * amount);
        ...
        public async Task<RuntimeResult> GetPointsAsync(IUser user)
        {
            try
            {
                CustomRuntimeResult<int> getResult = await Server.GetModule<CurrencyHandler>().GetPointsAsync(user);
