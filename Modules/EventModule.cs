...
[RequireUserPermission(GuildPermission.Administrator, Group = "EventPermissions")]
[RequireUserPermission(GuildPermission.ManageChannels, Group = "EventPermissions")]
...
public class EventModule : ModuleBase<CommandContext>, ICommandModule
{
    ...
    public async Task<RuntimeResult> AddActionEvent([Summary("Repetition Option")] ScheduledEvent.RepetitionOption repeat, [Summary("That date the event is being executed.")] DateTime executeAt,
                                                    [Summary("The event name.")] [RequireParameterLength(MIN_EVENT_NAME_LENGTH, MAX_EVENT_NAME_LENGTH)]
                                                    string eventTitle, string[] commands, IChannel channel = null)
    {
