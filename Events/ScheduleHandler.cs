...
public class ScheduleHandler : BaseHandler
{
    ...
    {
        StringBuilder builder = new("Events ERROR:\n");
            ...
            if (ev != null && ev.CommandToExecute != null)
            {
                if (!string.IsNullOrWhiteSpace(ev.CommandToExecute.Name))
                {
                    if (ev.Repetition != ScheduledEvent.RepetitionOption.Once || DateTime.Compare(ev.ExecutionTime, DateTime.Now) > 0)
                    {
                        ...
                        {
                            StringBuilder subSubBuilder = new($" | {ev.CommandToExecute.Name} | Commands ERROR:\n");
    ...
    {
        var loadedSchedule = await GenericXmlSerializer.LoadAsync<List<ScheduledEvent>>(Server.LogHandler, SCHEDULE_FILE_NAME, Server.ServerFilesDirectoryPath);
