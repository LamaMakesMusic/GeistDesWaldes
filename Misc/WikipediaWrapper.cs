...
namespace GeistDesWaldes.Misc
{
    public class WikipediaWrapper
    {
        ...
        {
            var result = await EnsureEntryCache();
            ...
            if (result.IsSuccess)
            {
                try
                {
                    ...
                    List<SectionContent> temp = new List<SectionContent>();
        ...
        private static async Task<CustomRuntimeResult<SectionContent[]>> EnsureEntryCache()
        {
            ...
            if (updateCache)
            {
                var result = await UpdateCache(DateTime.Today);
        ...
        private static async Task<CustomRuntimeResult> UpdateCache(DateTime date)
        {
            try
            {
                ...
                foreach (SectionInfo parentSection in sections)
                {
                    ...
                    {
                        await Launcher.Instance.LogHandler.Log(new Discord.LogMessage(Discord.LogSeverity.Debug, nameof(UpdateCache), $"Found Section: {section.Name}"));
                        ...
                        try
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(webString))
                            {
                                await Launcher.Instance.LogHandler.Log(new Discord.LogMessage(Discord.LogSeverity.Info, nameof(UpdateCache), $"Failed downloading webstring for section '{section.Name}'"));
                                continue;
                            }
                            ...
                            {
                                await Launcher.Instance.LogHandler.Log(new Discord.LogMessage(Discord.LogSeverity.Debug, nameof(UpdateCache), $"--- Found Entry: {content.Year} -> {content.Content}"));
                        ...
                        {
                            await Launcher.Instance.LogHandler.Log(new Discord.LogMessage(Discord.LogSeverity.Error, nameof(WikipediaWrapper), e.ToString()));
                        }
                    }
                }
        ...
        private static Task<SectionInfo[]> ParseSections(XmlDocument doc)
        {
            return Task.Run(() =>
            {
                ...
                foreach (XmlElement section in nodes?[0]?.ChildNodes)
                {
                    ...
                    SectionInfo info = new SectionInfo(section.GetAttribute("index"), section.GetAttribute("line"));
        ...
        private static Task<SectionContent[]> ParseSectionContents(XmlDocument doc, SectionInfo sectionInfo)
        {
            return Task.Run(() =>
            {
                ...
                foreach (Match entry in Regex.Matches(parsedNode, REGEX_ENTRY, RegexOptions.None))
                {
                    ...
                    SectionContent content = new SectionContent(sectionInfo.Name, sectionInfo.Type);
        ...
        private class SectionInfo
        {
            ...
            public List<SectionInfo> SubSections = new List<SectionInfo>();
