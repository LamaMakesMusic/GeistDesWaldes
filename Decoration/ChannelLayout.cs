    ...
    private async Task<CustomRuntimeResult> ModifyChannel(string newChannelName, string newChannelTopic)
    {
        try
        {
            ...
            if (cachedChannel is SocketTextChannel textChannel)
            {
                ...
                (string name, string topic) checkInfo = GetChannelInfo();
            ...
            if (cachedChannel is SocketVoiceChannel voiceChannel)
            {
                await voiceChannel.ModifyAsync(v => { v.Name = newChannelName; },
                    requestOptions);
