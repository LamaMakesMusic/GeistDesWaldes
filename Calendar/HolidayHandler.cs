...
public class HolidayHandler : BaseHandler
{
    ...
    private async Task CheckIntegrity()
    {
        ...
        {
            StringBuilder builder = new($"...[{behaviour.HolidayName}] ");
            int startLength = builder.Length;

            KeyValuePair<DateTime, string> correspondingDay = allHolidays.FirstOrDefault(h => h.Value.Equals(behaviour.HolidayName, StringComparison.OrdinalIgnoreCase));
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                StringBuilder subBuilder = new($"......[{i}]");
        ...
        {
            StringBuilder builder = new("Holiday Behaviours ERROR:\n");
    ...
    public static Task<Holiday[]> GetUpcomingHolidays(int maxEntries)
    {
        return Task.Run(() =>
        {
            IList<Holiday> holidays = GermanHolidays.GetHolidaysInDateRange(DateTime.Today, DateTime.Today.AddMonths(12));
    ...
    public Task<HolidayBehaviour[]> GetBehaviours(int maxEntries)
    {
        return Task.Run(() =>
        {
            var result = new List<HolidayBehaviour>();
    ...
    public async Task<CustomRuntimeResult> SetHolidayBehaviour(string holidayName, CustomCommand callback, HolidayBehaviour.BehaviourAction actionType)
    {
        ...
        if (result.IsSuccess)
        {
            HolidayBehaviour newBehaviour = new(result.ResultValue);
